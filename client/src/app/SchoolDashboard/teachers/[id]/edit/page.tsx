...
export default function EditTeacher() {
  ...
  useEffect(() => {
    const fetchTeacher = async () => {
      ...
      setError(null);
      try {
        const res = await window.authFetch(`/api/teachers/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await window.authFetch(`/api/teachers/${id}`, {
        method: "PUT",
              ...
              <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                ...
                {['teacher', 'school'].includes(userRole || '') ? (
                  ...
                  <motion.div className="form-field" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.55, duration: 0.6 }}>
                    ...
                    <div className="w-full bg-white/60 border border-blue-200 text-red-400 rounded-xl px-4 py-3 text-sm sm:text-base">Insufficient permissions to edit address</div>
                  </motion.div>
                )}
