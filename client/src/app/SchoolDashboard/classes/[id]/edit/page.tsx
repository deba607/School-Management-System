      ...
      try {
        ...
        if (data.success && data.data) {
          setForm({
            ...
            teacher: data.data.teacher?._id || data.data.teacher || "",
            day: data.data.day || [],
          ...
          setError(data.error || "Failed to fetch class schedule");
        }
      } catch (err: any) {
        setError(err.message || "Failed to fetch class schedule");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || "Failed to update class schedule");
      }
      setSuccess(true);
            ...
            {loading ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                >
                  {saving ? "Saving..." : "Update Class Schedule"}
                </motion.button>
                {success && (
                  <motion.div
